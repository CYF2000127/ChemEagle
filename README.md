...
|**OPSIN** |  Web Service for Name2SMILES                     | Converts chemical names to SMILES strings https://opsin.ch.cam.ac.uk/opsin/ (the three name services below can be turned off together with `use_network=False`; a local OPSIN answers without them) |
...
#### Sep 21 Update: The reaction and structure agents no longer transcribe SMILES. They receive the figure with numbered molecule boxes and answer with ids, and the program copies every structure from the vision result, so a model can no longer rewrite a molecule it was asked to identify. The molecule agent's edit plan is checked before it is applied (label OCR corrections whatever section they are filed under, variants bound by position, a terminal metal corrected to a placeholder the scheme draws elsewhere), and a set of deterministic repairs runs on the vision graphs themselves: drawing dirt read as a loose atom, misplaced ring double bonds, a charge or a radical the drawing does not show (a thiol read as a thiolate, an isocyanide as a nitrilium), and a graph that has run away, whose SMILES the model would otherwise copy and carry on until its answer is cut off. Where the two vision passes disagree about a box, the reaction agent's reading is kept when the disagreement is only about bond orders. Condition names resolve for every chemical role, and `ChemEagle(..., use_network=False)` or `CHEMEAGLE_NETWORK=0` turns the name services off, leaving the alias map, the cache and the local OPSIN to answer. Unit tests for the newer repairs are in `tests/`.
...
``` 
{"reactions":[
{"reaction_id":"0_1","reactants":[{"smiles":"[Ar]C([R])=C=O","smiles_parsable":"O=C=C(*)*","label":"1"},{"smiles":"Cc1ccc(S(=O)(=O)N2OC2c2ccccc2Cl)cc1","label":"2"}],
"conditions":[{"role":"reagent","text":"10 mol% B17 or B27","smiles":"C(c1ccccc1)[N+]1=CN2[C@H](C(O[Si](C)(C)C)(c3ccccc3)c3ccccc3)CCC2=N1.F[B-](F)(F)F","label":"B17"},
{"role":"reagent","text":"10 mol% B17 or B27","smiles":"CC(C)c1ccccc1[N+]1=CN2[C@H](C(O)(c3cc(C(F)(F)F)cc(C(F)(F)F)c3)c3cc(C(F)(F)F)cc(C(F)(F)F)c3)CCC2=N1.F[B-](F)(F)F","label":"B27"},
